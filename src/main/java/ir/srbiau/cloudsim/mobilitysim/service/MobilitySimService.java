...
import ir.srbiau.cloudsim.mobilitysim.dto.response.CloudletStatus;
...
public class MobilitySimService {
    ...
    public ResponseEntity<SimulationResultDto> run(ResourceRequestDto dto) {
        try {
            ...
            return ResponseEntity.ok().body(new SimulationResultDto(buildCloudletStatuses(cloudletFinished)));
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    private List<CloudletStatus> buildCloudletStatuses(List<CloudletMobility> cloudletMobilities) {
        DecimalFormat dft = new DecimalFormat("###.##");
        return cloudletMobilities.stream().map(
                c -> new CloudletStatus(
                        c.getCloudletId(),
                        c.getStatus().name(),
                        c.getGuestId(),
                        dft.format(c.getExecStartTime()),
                        dft.format(c.getExecFinishTime())
                )
        ).toList();
    }
